...
static struct virtual_usb_udc *the_udc_pdev[MAX_NUM_UDC];
...
static int __init init(void)
{
	...
	for (i = 0; i < mod_data.num; i++) {
		the_udc_pdev[i] = virtual_usb_alloc_udc(gadget_name, i);
		if (IS_ERR(the_udc_pdev[i])) {
			i--;
			while (i >= 0)
				virtual_usb_put_udc(the_udc_pdev[i--]);
			goto err_alloc_udc;
		}
	}
	...
	for (i = 0; i < mod_data.num; i++) {
		...
		retval = virtual_usb_udc_add_data(the_udc_pdev[i], &dum[i],
						  size(void *));
